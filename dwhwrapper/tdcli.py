...
class dbc_connection:
    ...
    def logon(self,dbc_name,uid,password):
        ...
        lstr = '{0}/{1},{2}'.format(dbc_name,uid,password)
    ...
    def fetch_request(self,return_on_parcels=[]):
        ...
        while True:
            ...
            if self.dbcarea.fet_parcel_flavor in return_on_parcels:
                ...
                temp = "STATEMENT: " + str(cf.StatementNo) + ", ERR: " + str(cf.Code) +'\n' + str(cf.Msg[:cf.Length])
...
#                 raise Exception("STATEMENT:{0} ERR:{1} {2}".format(cf.StatementNo,cf.Code,cf.Msg[:cf.Length]))
    ...
    def close_request(self):
        ...
        #Zack altered dbc.msg_text to self.dbcarea.msg_text
        if self.result != self.EM_OK:
            raise Exception("End req. failed {0}".format(self.dbcarea.msg_text))
...
class type_char(td_type):    
    ...
    def pack(self,rph,r):
        ...
        #Zack - edited array from d['Name] to self.fd['Name']
        if len(r) > self.fd['Len']:
            raise ValueError("Column '{0}' contains a string longer than {1} characters".format(self.fd['Name'],self.fd['Len']))
...
class indic_data:
    ...
    def pack(self,nulls):
        ...
        #Zack, altered to while loop
        i = 0
        while i < self.indic_data_len:
#         for i in range(0,self.indic_data_len):
        ...
        return b''.join(chr(b) for b in myBytes)
...
class row_pack_handler:
    ...
    def pack_row(self,column_count):
        ...
        myId = indic_data(column_count)
        id_bytes = myId.pack(nulls=self.nulls)
...
class row_unpack_handler:
    ...
    def __init__(self,row_data,columns):
        ...
        myId = indic_data(columns)
        self.nulls = myId.unpack(row_data)
        self.offset = myId.indic_data_len
...
def fexp_to_csv(ddf,fexp_file,args):
    ...
    with file(args.output,'w') as out_file:
        ...
        with open(fexp_file,'rb') as myInput:
            
            while True:
                
                header = myInput.read(2)
                if len(header) < 2:
                    #EOF
                    break
                
                row_len = struct.unpack('H',header)[0]
                row_data = myInput.read(row_len)
                ...
                myInput.read(1) #End of record indicator is a single newline char..
